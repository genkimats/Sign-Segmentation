"""
train_disk.py -- IDENTICAL to train.py in every way except the dataset class:
uses SignSegmentationDatasetDisk (reads features from disk per-item, small
bounded LRU cache) instead of SignSegmentationDataset (loads the entire
dataset into a RAM cache at startup).

Use this INSTEAD OF train.py when your feature combination doesn't fit in
memory -- e.g. DINOv2 features at ~100GB against 28GB RAM. Everything else
(the restart-on-NaN loop, hardware_summary.json, metrics CSV, seeding,
loss functions, model registry) is unchanged -- this pulls from the SAME
train_queue.json queue_train.py builds, so no change to your queueing
workflow is needed. Just run this script instead of train.py when you want
to process a job that needs disk-mode loading.

Expect SLOWER epochs than the RAM-cached path reports for otherwise-identical
configs -- this trades speed for feasibility, not the other way around. See
the chat message this was delivered in for concrete alternatives (smaller
DINOv2 variant, fp16 storage, PCA-reduced features) that might let a future
run fit in the RAM-cached dataset instead.
"""
import time
import json
import csv
...
from src.dataset_disk import SignSegmentationDatasetDisk
...
def train_model(config):
    ...
    train_dataset = SignSegmentationDatasetDisk(
        ...
        cache_size=config.get("disk_cache_size", 2)
    )
    
    val_dataset = SignSegmentationDatasetDisk(
        keypoints_dir="processed_data/keypoints",
        labels_dir="processed_data/BIO_tags",
        split_file="dataset_splits.json",
        split="val",
    ...
    # num_workers=4 here means up to 4 SEPARATE per-worker LRU caches (see
    # SignSegmentationDatasetDisk's docstring) -- if RAM is genuinely tight even
    # with disk-mode loading, lowering num_workers is another lever, at the cost
    # of less I/O parallelism (slower epochs).
